...
#define CL_LOADER_LOG(...) __android_log_print(ANDROID_LOG_VERBOSE, "libopencl_loader", __VA_ARGS__)
#define CL_LOADER_LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, "libopencl_loader", __VA_ARGS__)
#define CL_LOADER_LOGW(...) __android_log_print(ANDROID_LOG_WARN, "libopencl_loader", __VA_ARGS__)
#define CL_LOADER_LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "libopencl_loader", __VA_ARGS__)
#else
 ...
 */
using clBuildProgram_func =
    cl_int (*)(cl_program, cl_uint, const cl_device_id*, const char*, void (*pfn_notify)(cl_program, void*), void*);
...
using clEnqueueWriteBuffer_func =
    cl_int (*)(cl_command_queue, cl_mem, cl_bool, size_t, size_t, const void*, cl_uint, const cl_event*, cl_event*);
using clEnqueueReadBuffer_func =
    cl_int (*)(cl_command_queue, cl_mem, cl_bool, size_t, size_t, void*, cl_uint, const cl_event*, cl_event*);
...
using clCreateProgramWithBinary_func =
...
using clGetSupportedImageFormats_func =
    cl_int (*)(cl_context context, cl_mem_flags, cl_mem_object_type, cl_uint, cl_image_format*, cl_uint*);
...
using clCreateContextFromType_func =
...
using clCreateContext_func = cl_context (*)(const cl_context_properties*,
                                            cl_uint,
                                            const cl_device_id*,
...
using clEnqueueFillBuffer_func =
    cl_int (*)(cl_command_queue, cl_mem, const void*, size_t, size_t, size_t, cl_uint, const cl_event*, cl_event*);
using clCreateImage_func =
...
using clEnqueueMapImage_func = void* (*)(cl_command_queue,
                                         ...
                                         cl_event*,
                                         cl_int*);
using clGetKernelWorkGroupInfo_func =
...
using clCreateImage2D_func =
    cl_mem (*)(cl_context, cl_mem_flags, const cl_image_format*, size_t, size_t, size_t, void*, cl_int*);
...
public:
    ...
    int Init() {
        ...
        if (CLSymbols::Get().clBuildProgram == nullptr || CLSymbols::Get().clEnqueueNDRangeKernel == nullptr ||
            CLSymbols::Get().clSetKernelArg == nullptr || CLSymbols::Get().clReleaseKernel == nullptr ||
            CLSymbols::Get().clCreateProgramWithSource == nullptr || CLSymbols::Get().clCreateBuffer == nullptr ||
            CLSymbols::Get().clRetainKernel == nullptr || CLSymbols::Get().clCreateKernel == nullptr ||
            CLSymbols::Get().clGetProgramInfo == nullptr || CLSymbols::Get().clFlush == nullptr ||
            CLSymbols::Get().clReleaseProgram == nullptr || CLSymbols::Get().clRetainContext == nullptr ||
            CLSymbols::Get().clCreateProgramWithBinary == nullptr ||
            CLSymbols::Get().clReleaseCommandQueue == nullptr || CLSymbols::Get().clEnqueueMapBuffer == nullptr ||
            CLSymbols::Get().clRetainProgram == nullptr || CLSymbols::Get().clGetProgramBuildInfo == nullptr ||
            CLSymbols::Get().clEnqueueReadBuffer == nullptr || CLSymbols::Get().clEnqueueWriteBuffer == nullptr ||
            CLSymbols::Get().clReleaseEvent == nullptr || CLSymbols::Get().clRetainCommandQueue == nullptr ||
            CLSymbols::Get().clEnqueueUnmapMemObject == nullptr || CLSymbols::Get().clReleaseMemObject == nullptr ||
            CLSymbols::Get().clCreateCommandQueue == nullptr || CLSymbols::Get().clCreateContextFromType == nullptr ||
            CLSymbols::Get().clGetContextInfo == nullptr || CLSymbols::Get().clGetDeviceInfo == nullptr ||
            CLSymbols::Get().clGetPlatformIDs == nullptr || CLSymbols::Get().clGetPlatformInfo == nullptr ||
            CLSymbols::Get().clRetainDevice == nullptr || CLSymbols::Get().clReleaseDevice == nullptr ||
            CLSymbols::Get().clCreateContext == nullptr || CLSymbols::Get().clFinish == nullptr ||
            CLSymbols::Get().clGetDeviceIDs == nullptr || CLSymbols::Get().clCreateImage == nullptr ||
            CLSymbols::Get().clEnqueueMapImage == nullptr || CLSymbols::Get().clGetKernelWorkGroupInfo == nullptr ||
            CLSymbols::Get().clWaitForEvents == nullptr || CLSymbols::Get().clGetEventProfilingInfo == nullptr ||
            CLSymbols::Get().clCreateImage2D == nullptr || CLSymbols::Get().clRetainMemObject == nullptr ||
            CLSymbols::Get().clRetainEvent == nullptr || CLSymbols::Get().clEnqueueReadImage == nullptr ||
            CLSymbols::Get().clEnqueueWriteImage == nullptr || CLSymbols::Get().clGetSupportedImageFormats == nullptr) {
...
CL_LOADER_EXPORT cl_int clBuildProgram(cl_program program,
                                             cl_uint num_devices,
...
CL_LOADER_EXPORT cl_program clCreateProgramWithBinary(cl_context context,
                                                            cl_uint num_devices,
                                                            const cl_device_id* device_list,
...
CL_LOADER_EXPORT cl_kernel clCreateKernel(cl_program program, const char* kernel_name, cl_int* errcode_ret) {
...
CL_LOADER_EXPORT cl_program clCreateProgramWithSource(
...
CL_LOADER_EXPORT cl_command_queue clCreateCommandQueue(cl_context context,
                                                             cl_device_id device,
...
CL_LOADER_EXPORT cl_int clGetSupportedImageFormats(cl_context context,
                                                         cl_mem_flags flags,
...
CL_LOADER_EXPORT cl_int clGetPlatformIDs(cl_uint num_entries, cl_platform_id* platforms, cl_uint* num_platforms) {
...
CL_LOADER_EXPORT cl_int clGetKernelWorkGroupInfo(cl_kernel kernel,
                                                       cl_device_id device,
...
CL_LOADER_EXPORT cl_int clWaitForEvents(cl_uint num_events, const cl_event* event_list) {
